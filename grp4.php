...
// $vars = 10;
for($i=2;$i<=$vars;$i++){
$sql .="(SELECT ".$i." AS var, sa.gridcode FROM presion_".$i." sa WHERE sde.st_intersects(sde.st_point (".$lon.", ".$lat.", 4326), sa.shape))";
if ($i<$vars){ $sql .="UNION"; };
}	
$sql .="ORDER BY var;";
// echo $sql;		
...
@$dd1 	= array_map( create_function('$value', 'return 950+(int)$value;'), $d1);
...
<div id="container4" style="min-width: 310px; max-width: auto; height: 300px; margin: 0 auto"></div>
